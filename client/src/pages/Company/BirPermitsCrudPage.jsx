...
export default function BirPermitsCrudPage() {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      try {
        ...
        const terminalTypesPromise = apiWithCredentials.get('/v2/api/bir-permit-types');
        const serversPromise = apiWithCredentials.get('/v2/api/servers');

        const [permitsResponse, terminalTypesResponse, serversResponse] = await Promise.all([permitsPromise, terminalTypesPromise, serversPromise]);
        ...
        if (error.response && error.response.config.url.includes('/v2/api/bir-permits')) {
          console.error('Error fetching permits:', error);
        } else if (error.response && error.response.config.url.includes('/v2/api/bir-permit-types')) {
          console.error('Error fetching terminal types:', error);
        } else if (error.response && error.response.config.url.includes('/v2/api/servers')) {
  ...
  const handleFormSubmit = async (data) => {
    ...
    try {
      if (editMode) {
        ...
        const response = await apiWithCredentials.put('/v2/api/bir-permits', { ...permitData, ...data });
        console.log('API response for updating permit:', response.data);
      } else {
        // Create permit
        const response = await apiWithCredentials.post('/v2/api/bir-permits', data);
      ...
      console.error(`Error ${editMode ? 'updating' : 'creating'} permit:`, error);
    }
  };
  ...
  const handleDelete = async (id) => {
    try {
      await apiWithCredentials.delete('/v2/api/bir-permits', { data: { terminalid: id } });
      fetchPermits(); // Refresh permits list
    } catch (error) {
